...
def napsa_report(dbms, object):
    ...
    napsa = DataConversion.safe_get (filter_obj, "napsa")
    ...
    if emp_name:
        ...
        employee = utils.array_to_dict (core_hr.get_list ("Employee", {"napsa": napsa}, limit=1), "name")
    else:
        employee = utils.array_to_dict (core_hr.fetch_data_from_sql ("SELECT napsa, name, d_o_b, id_no, first_name, middle_name, last_name FROM employee"), "name")
                            ...
                            if name not in track_emp:
                                ...
                                if em:
                                    ...
                                    if first_name and last_name:
                                        ...
                                        DataConversion.safe_set (report_row, "company_napsa_acc", DataConversion.safe_get (hr_settings, "company_napsa_acc", ""))
                                        ...
                                        DataConversion.safe_set (report_row, "napsa", DataConversion.safe_get (em, "napsa"))
                                        napsa_data_result.append (report_row)
                except Exception as err:
                    pp (f"{str (err)}")
                    pass

    return utils.respond(utils.ok, {'rows': napsa_data_result})
        ...
        select_field?.off('change').on('change', async(e) => {
            ...
            if(prev_val != value || is_first_value === true){
                ...
                if (!this.utils.is_empty_array(this.on_select_change)) {
                    ...
                    // add table field content
                    if($(e.currentTarget)?.hasClass("table-field")){
                        obj.istablefield = true
                        obj.fieldrow = $(e.currentTarget).parents(".tr.table-row")
                        obj.table_name = $(e.currentTarget).parents(".table")?.attr("fieldname")
                        obj.row_id = $(e.currentTarget).parents(".tr.table-row")?.attr("id")
                    }
            ...
            if (!this.utils.is_empty_array(fetch_from_fields) && model) {
                ...
                if (!lite.utils.is_empty_object(doc)) {
                    $.each(fetch_from_fields, (_, f) => {
                        ...
                        if(!lite.form_controller?.is_child_table_field(f)){
                            if (fetch_field) {
                            ...
                            const row_id = $(f)?.parents(".table-row")?.attr("id"), fieldname = $(f)?.attr("fieldname"), table_name = $(f)?.parents(".table")?.attr("fieldname")
                            lite.form_controller?.set_form_table_value(table_name,row_id,fieldname,doc[fetch_field],null, true)
                        }
                    })
                }
                else {

...
export default class Form_Controller {
    constructor(config) {
        ...
        this.workflow = {}
        this.common_field_types = ["text", "longtext", "int", "currency", "percentage", "float","password","color"]
        this.numeric_fields_types = ["int", "currency", "percentage", "float",]
        this.selected_items = []
        this.table_actions = {}
    ...
    async #append_field(field_info){
        ...
        if (lite.utils.object_has_data(this.doc_data)) {
            if(field_info.fieldtype !== "table"){
                ...
                if(this.doc_data[field_info.fieldname]){
                    field_info.rows = this.doc_data[field_info.fieldname]
                }
            }
        }
        else{
            ...
            $(wr).find(this.row_check_box_class_name)?.unbind("change")?.change(e => {
                ...
                if (for_row == 'head-row') {
                    if ($(e.target).prop('checked')) {
                        ...
                        console.log($(wr).find(this.head_row_check_box_class_name))
                    }
                }
                if (!$(e.target).prop('checked')) {
    ...
    #init_input_focus_event() {
        // this.$form_fields.find(this.table_wrapper_class_name).find('input').focus(e => {
        //     console.log($(e).parents('.table-row'))
        // })
    }
    ...
    #init_add_remove_row_action() {
        ...
        this.$form_fields.find(this.table_wrapper_class_name)?.map((_, wr) => {
            const model_type = lite.utils.get_attribute(wr, 'for')
            ...
            $(wr).find(this.child_table_add_remove_action_btn)?.click(e => {
                e.preventDefault()
                const action = lite.utils.get_attribute(e.currentTarget, 'action'),
                    table = $(e.currentTarget).parent().siblings('.table')
                if (action === 'add') {
                    table.hasClass('border-red-600') ? table.removeClass('border-red-600') : ''
                    $(wr).find(this.table_body_class_name).append(this.html_bulder.create_table_row(this.child_tables[model_type], true, true))
                    ...
                    const table_dependants = this.$form.find(`div.th[display-on]`)
                    if(table_dependants && lite.utils.array_has_data(table_dependants)){
                        table_dependants.map((_,d)=>{
    ...
    async update_uploaded_files(data, validation){
        ...
        if(validation?.has_files){
            // console.log(validation?.processed_files)
        ...
        if(validation?.has_table_files){
            console.log(lite.lite_file_picker?.processed_form_files, data)
    ...
    async handle_select_change(data, cls) {
        ...
        if($(data.field)?.parents("form")?.attr("id") === $(class_obj.$form)?.attr("id")){
            const fieldname = lite.utils.get_attribute(data.field, "fieldname")
            const
                    istablefield = lite.utils.is_empty_array($(data.field).parents(".table-row")) ? false : true,
                    fieldrow = istablefield ? $(data.field).parents(".table-row")[0] : null,
                    row_id= $(fieldrow)?.attr("id"),
                    id = lite.utils.get_attribute(data.field,'id'),
                    value = data.value
                   
            if (cls.events[cls.model]?.on_field_change && !lite.utils.is_empty_array(cls.events[cls.model]?.on_field_change[fieldname])) {
                await Promise.all(cls.events[cls.model]?.on_field_change[fieldname]?.map(async f=>{
                    await f({
            ...
            cls.display_or_hide_dependant_fields(id, value)
        }
    }
                    ...
                    if (this.events[this.model]?.on_field_change && !lite.utils.is_empty_array(this.events[this.model]?.on_field_change[fieldname])) {
                        ...
                        $.each(this.events[this.model]?.on_field_change[fieldname], (_, f) => {
                            f({
                                fieldname: fieldname,
                                value: lite.utils.get_field_value(e.target),
                                fieldtype: lite.utils.get_attribute(e.target, "type"),
                                form: this.$form,
                                field: e.target,
    ...
    display_or_hide_dependant_fields(field_id, current_value, process_for_table_only=false){
        ...
        // table display on
        const table_dependants = this.$form.find(`div.th[display-on="${field_id}"]`)
        if(table_dependants && !lite.utils.is_empty_array(table_dependants)){
            $.each(table_dependants,(_,d)=>{
                const table_wrapper = $(table_dependants)?.parents(".table")
    ...
    async get_await_form_value(field){
       return await new Promise((resolve,reject) => {
            let timeOut = null
            let initial_value = this.get_form_value(field)
            const interval = setInterval(() => {
                let v = this.get_form_value(field)
    ...
    set_form_table_value(table_name, row_id, field, value, read_only_display_figure = null, trigger_change_event=true) {
        const field_type = lite.utils.get_attribute(field, 'type')
        if (this.common_field_types.includes(field_type)) {
            $(field).val(lite.utils.is_number_variable(value) && [] ? lite.utils.thousand_separator(value, lite.currency_decimals) : value)
            if(trigger_change_event){
                $(field).trigger("focusout")
            }
        } else if (field_type === 'checkbox') {
            ...
            lite.expandable_field.update_field(field, value)
        }
    }
    ...
    get_table_rows(table_name) {
        let rows = []
        const table_rows = this.$form.find(`.lite-field[fieldname="${table_name}"]`).find('.table-row')
        if (table_rows?.length > 0) {
            $.each(table_rows, (_, r) => {
                let row = {}
                $.each($(r).find("input.lite-field,textarea.lite-field,div.lite-field:not(.lite-selector)"), (_, f) => {
                    const field_type = lite.utils.get_attribute(f, 'type')
                    let fd = {
                        row_id: lite.utils.get_attribute(r, "id"),
                        field: f,
                        fieldname: lite.utils.get_attribute(f, 'fieldname'),
                        fieldtype: field_type
                    }
                    if (field_type === 'link' || field_type === 'select') {
                        fd.value = $(f).val()
                    } else if (this.common_field_types.includes(field_type)) {
                        fd.value = $(f).val()
                    } else if (field_type === 'checkbox') {
                        fd.value = $(f).prop('checked') ? 1 : 0
                    } else if (field_type === 'read-only' || field_type === "switch") {
                        fd.value = lite.utils.get_attribute(f, 'value')
                    } 
                    row[fd.fieldname] = fd
                })
                rows.push(row)
    ...
    get_table_column_values(table_name, column_names=[], detailed=false) {
        let columns = {}
        let names = ""
        $.each(column_names,(_, name)=>{
            names += `.lite-field[fieldname="${name}"], `
            columns[name] = [] 
        })
        const table_columns = this.$form.find(`.lite-field[fieldname="${table_name}"]`).find(`${lite.utils.remove_last_characters(names, 2)}`)
        if (table_columns?.length > 0) {
            $.each(table_columns, (_, c) => {
                let row = {}
                const field_type = lite.utils.get_attribute(c, 'type')
                let fd = {
                    row_id: lite.utils.get_attribute(c, "id"),
                    field: c,
                    fieldname: lite.utils.get_attribute(c, 'fieldname'),
                    fieldtype: field_type
                }
                if (field_type === 'link' || field_type === 'select') {
                    fd.value = $(c).val()
                    ...
                    fd.value = $(c).prop('checked') ? 1 : 0
                } else if (field_type === 'read-only' || field_type === "switch") {
                    fd.value = lite.utils.get_attribute(c, 'value')
                } 
                row[fd.fieldname] = fd
                if (fd.value){
                    detailed  ? columns[fd.fieldname].push(fd) : columns[fd.fieldname].push(fd.value)
                }
            })
        }
        return columns
    ...
    get_table_row(table_name,row_id, get_first=false, get_last=false) {
        let table_rows
        if (row_id){
            table_rows = this.$form.find(`.lite-field[fieldname="${table_name}"]`).find(`.table-row#${row_id}`)
        }
        else{
            if(get_first){
                table_rows = this.$form.find(`.lite-field[fieldname="${table_name}"]`).find(`.table-row`)?.first()
            }
            else if(get_last){
                table_rows = this.$form.find(`.lite-field[fieldname="${table_name}"]`).find(`.table-row`)?.last()
            }
        }
        
        if (table_rows?.length > 0) {
            let row = {}
            const row_html = table_rows[0]
            $.each($(row_html).find("input.lite-field,textarea.lite-field,div.lite-field:not(.lite-selector)"), (_, f) => {
                const field_type = lite.utils.get_attribute(f, 'type')
                let fd = {
                    row_id: lite.utils.get_attribute(row_html, "id"),
                    field: f,
                    fieldname: lite.utils.get_attribute(f, 'fieldname'),
                    fieldtype: field_type
                }
    ...
    populate_child_table(table_name, data, clear_before_populate=false){
        if(!lite.utils.is_empty_object(this.child_tables)){
            if(clear_before_populate){
                this.clear_table(table_name)
            }
            let field = {}
            $.each(lite.utils.get_object_keys(this.child_tables),(_,f)=>{
                if(this.child_tables[f]?.fieldname === table_name)
                    field = lite.utils.copy_object(this.child_tables[f])
            })
            if(!lite.utils.is_empty_object(field) && !lite.utils.is_empty_array(field?.fields)){
                let fields = []
                $.each(data,(_,d)=>{
                    let new_row = []
                    $.each(field.fields,(_,f)=>{
                        f.value = d[f["fieldname"]] || d[f["fieldname"]] == 0  ? d[f["fieldname"]] : ''
                        new_row.push(lite.utils.copy_object(f))
                    })
                    field.fields = new_row
                    fields.push(lite.utils.copy_object(field))
                })
                this.add_child_table_row(table_name,fields)
            }
        }
    }

    add_child_table_row(table_name, data){
        if(!lite.utils.is_empty_array(data)){
            let rows = ''
            $.each(data,(i,r)=>{
                rows += this.html_bulder.create_table_row(r,true)
            })
    ...
    update_actions() {
        this.dropdown_actions = {
            ...
            send_mail: { icon: 'mail', icon_color: 'violet', title: "Send Mail", action: null, for_docstatus: [0, 1]},
        }

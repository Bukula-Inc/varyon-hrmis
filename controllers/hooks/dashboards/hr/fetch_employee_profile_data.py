...
def fetch_employee_profile_data(dbms, object):
    ...
    try:
        ...
        fetch_petty_cash =dbms.get_list("ECZ_Petty_Cash", filters={"initiator": emp_info})
        if fetch_petty_cash.status ==utils.ok:
            petty_cash =fetch_petty_cash.data.rows

        fetch_imprest =dbms.get_list("ECZ_Imprest", filters={"initiator": emp_info})
        ...
        fetch_leave_entrires =dbms.get_list("Leave_Entry", filters={"employee": emp_info})
        ...
        fetch_work_plan =dbms.get_list("Performance_Agreement", filters={"employee_id":emp_info})
        pp(emp_info, fetch_work_plan)
    ...
    returned_data.top_cards =utils.from_dict_to_object({
        ...
        "years_of_service": int(dates.date_difference(str(emp.date_of_joining), str(dates.today()))/365)
    })
    ...
    returned_data.appraisal =utils.from_dict_to_object({        
        "distribution": expense_distribution,
    })

import Bank_Transactions from '../modules/accounting/banking/bank_transactions.js';
export default class Bank_Transaction_Actions{
    ...
    async workflow_comments_transactions(id){
    ...
    init(){
        $(document).on("click", '.bank-transaction-action-btn', async (e) => {
            ...
            if(status == lite.status_codes.ok){
                ...
                if(lite.utils.array_has_data(workflow.available_actions)){
                    ...
                    if (!workflow?.has_workflow){
                        if(data.docstatus === 0 && data.status == "Draft"){
                            ...
                            options = this.generation_no_actions_html()
                        }
                        
                    }
                }
                options += `
                    ${this.generate_action_html(data.id, "Audit Trail", 0, "clear_all", "#7a0382", "#7a0382", false)}
                    ${this.generate_action_html(data.id, "Workflow Comments", 0, "comment", "#039979", "#039979", false)}
                `
                $(`#bank-transaction-options-${id}`).html(options)
            }
        })

        // for direct actions where wf is none applicable

...
export const witness_func = async (params) => {
    const { controller } = params
    const url_params = lite.utils.get_url_parameters ()

    if (!url_params?.doc) {
        lite.alerts.toast({toast_type:lite.status_codes.no_content, title:"Doc To Witness", message:`the document is Missing. Ensure the Doc Name is Available`})
        return
    }

    const loader_id = lite.alerts.loading_toast({title: `Authenticating Doc`, message:"Please wait while we authenticate the document."})
    controller.set_form_value (controller.get_form_field ("doc"), url_params?.doc)
    const get_doc = await lite.connect.x_post ("witness", {doc: url_params?.doc})

    lite.alerts.destroy_toast(loader_id)
    
    if (get_doc.status == lite.status_codes.ok) {
        const data = get_doc.data
        const applicant = data?.linked_fields.applicant
        
        controller.set_form_value (controller.get_form_field ("document_type"), data?.document_type)
        controller.set_form_value (controller.get_form_field ("employee"), applicant?.name)
        controller.set_form_value (controller.get_form_field ("full_names"), applicant?.full_name)
    }
}
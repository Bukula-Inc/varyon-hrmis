export default class Validator {
    ...
    #clean_value(val, field){
        if(lite.utils.is_figure_value(val) && $(field)?.attr("fieldtype") != "text"){
            return lite.utils.string_to_float(val)
        }
        else if(lite.utils.is_integer_value(val) && $(field)?.attr("fieldtype") != "text"){
    ...
    #get_fields(return_if_validation_failed, ignore_validation = false, is_saving_or_updating) {
        ...
        if (!lite.utils.is_empty_array(fields)) {
            ...
            $.each(fields, (_, f) => {
                ...
                if ((!`${d.value}`.trim() && d.value !== `${0}`) && d.rqd && !ignore_validation) {
                    this.has_passed = false
    ...
    #get_field_data(field, ignore_validation = false, is_saving_or_updating) {
        let value = ''
        const direct_fields = ['text', 'int', 'date', "time", "float", 'barcode', "hidden", "currency", "percentage","password","color", "image"]
        const field_type = lite.utils.get_attribute($(field), 'fieldtype')
        const is_hidden = $(field).hasClass('hidden')
        const fieldname = lite.utils.get_attribute($(field).find('input:not(.table-search-field)'), 'fieldname') || lite.utils.get_attribute($(field), 'fieldname') || lite.utils.get_attribute($(field).find('.lite-field:not(.table-search-field)'), 'fieldname')
        if (direct_fields.includes(field_type)) {
            ...
            if (!lite.utils.is_empty_array(rows)) {
                        ...
                        if (!lite.utils.is_empty_array(cells)) {
                            $.each(cells, (_, c) => {
                                ...
                                if (direct_fields.includes(fieldtype)) {
                                    ...
                                    // console.log(lite_field.attr("id"), lite_field.attr("lite-id"))
        ...
        // }
        // if(!fieldname){
            let fn  = lite.utils.get_attribute($(field).find('input'), 'fieldname') || lite.utils.get_attribute($(field), 'fieldname') || lite.utils.get_attribute($(field).find('.lite-field'), 'fieldname')
        // }
        return {

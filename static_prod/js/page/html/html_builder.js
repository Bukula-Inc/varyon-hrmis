export default class HTML_Builder {
    ...
    build_form_field(params, setup) {
        ...
        const lb = fieldtype != 'check' ? fieldlabel || placeholder : ""
    ...
    input_float_field(opts) {
        return `
            <input 
                ...
                is_required="${opts?.required || false}" 
                type="float"
                ${opts?.value ? 'value="' + lite.utils.thousand_separator(opts.value,2) + '"' : ''}
                ${opts?.value ? 'prev-val="' + lite.utils.thousand_separator(opts.value,2) + '"' : ''}
                fieldname="${opts?.fieldname}"
                ${opts?.displayon ? "display-on='"+ opts.displayon[0] +"'" : ''}
                ${opts?.displayon ? "display-eval='"+lite.utils.object_to_string(opts.displayon)+"'" :''}
                ${opts.fetchfrom ? 'fetch-from="' + opts.fetchfrom + '" fetch-field="' + opts.fetchfield + '"' : ""}
    ...
    read_only_field(opts) {
        ...
        if(opts?.is_figure || lite.utils.is_number_variable(opts?.value))
    ...
    create_child_table(opts, cols = 2) {
        ...
        if (!lite.utils.is_empty_array(opts?.rows)) {
            let row_data = lite.utils.ascend(opts?.rows, "id")
            $.each(row_data, (_, r) => {
                let opt = {value:r, idx:_, ...lite.utils.copy_object(opts)}
                $rows += this.create_table_row(opt, editable)
            })
        }
        else {
            opts.idx = 0
            opts.value = ''
            $rows = (opts?.addemptyrow === undefined || opts?.addemptyrow) ? this.create_table_row(opts) : ''
        }
        const
    ...
    #create_child_table_head(opts) {
        ...
        let head = `
            <div class="thead w-full h-[30px] text-13" cols="${cols} ${scrollable ? 'flex items-center justify-start flex-row': ''}">
    ...
    create_table_row(opts, editable = true, is_new = false) {
        let cols = 2
        $.each(opts?.fields, (_, f) => {
            // if(lite.utils.is_object(f.displayon) && lite.utils.array_length(f.displayon) > 1){
            //     const ev = f.displayon.slice(1), key = f.displayon[0]
            //     if(lite.form_data && ev?.includes(lite.form_data[key])){
            //         f.hidden = false
            //     }
            //     else{
            //         f.hidden = true
            //     }
            // }
            cols += f?.hidden ? 0 : parseInt(f.columns || 1)
        })
        // console.log(lite.form_data)
        let row = `
            <div id="${lite.utils.unique()}" doc-id="${opts?.value?.id || null}" cols="${cols}" class="tr table-row hover:bg-default/30 bg-slate-50 intro-x w-full h-full grid grid-cols-${cols} text-slate-700 border-b border-dashed min-h-[40px] ">
    ...
    #create_child_table_action() {
        return `
            <div class="w-full flex items-center justify-start h-[30px] rounded-b-md mt-2">
    ...
    create_header_row(cols, html) {
        return `
            <div class="w-full lhead gap-x-3 bg-default h-[35px] rounded-t-md text-theme_text_color grid grid-cols-${cols} font-normal border intro-x text-11">
                ${html}
    ...
    create_list_row(sn, row, columns, actions, settings) {
        ...
        for (const c of columns) {
            ...
            if (c.column_type === "check") {
                ...
                v = `<img src="${value || "/media/defaults/avatas/dp.jpeg"}" class="w-[80%] px-3 h-[35px] rounded-md shadow-md border object-contain object-center"/>`;
    ...
    create_pagination(params) {
        const { total_pages, current_page, page_size, total_records,queried_total, page_range, } = params
        let page_list = ''
        let page_options = ''
            ...
            page_list += `
                <li class="page-item transition duration-1000 ${page !== parseInt(current_page) ?'intro-x': '' }"> 
                    <a class="page-link paginate-select text-12 ${page === parseInt(current_page) ? 'active bg-default text-theme_text_color' : ''}" index="${page}">
                        <span class="${page === parseInt(current_page) ? 'text-white text-30 font-bold' : ''}">${page}</span>
                    </a> 
                </li>
            `
            page_options += `<option value="${page}">Page ${page}</option>`
        })
        return `
            <div class="w-full pagination_wrapper col-span-12 flex flex-wrap items-center bg-white h-[40px] p-3 mb-3">
    ...
    #create_report_body(setup) {
        return `<div class="table-body min-h-[30vh]  ${setup?.is_grid_layout ? "w-full" : "w-[max-content]"} "></div>`
    }
    ...
    create_report_row(sn, data, columns, setup) {
        ...
        $.each(columns, (_, c) => {
            ...
            if(c.formatter && c.apply_formatter_on_total && data?.total_fields?.includes(c.column_name)){
                v = c.formatter(data[c.column_name])
            }
                ...
                if (c.is_figure && !data?.is_head) {
                    if (v)
                        if(lite.utils.is_number_variable(v))
                            v = lite.utils.thousand_separator(data[c.column_name], lite.defaults?.currency_decimals)
                        else v = ""
                    else
    ...
    create_save_button(attr) {
        return `
            <button class="save-action-btn ${attr?.classname} min-w-[160px] h-[47px] text-16 max-w-[max-content]  btn bg-default text-theme_text_color shadow-md ">
                <span class="material-symbols-outlined mr-2 text-18"> task_alt</span> Save
            </button>               
        `
    }
    ...
    create_update_button(data, attr) {
        ...
        return `
            <button class="update-action-btn ${attr?.classname} min-w-[150px] h-[47px] text-16 font-semibold max-w-[max-content]  btn bg-default text-theme_text_color shadow-md ml-2">
                <span class="material-symbols-outlined mr-2 text-18"> save</span> Update
            </button>               
        `
    }

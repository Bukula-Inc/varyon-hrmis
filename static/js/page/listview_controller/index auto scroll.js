...
export default class Listview_Controller {
    ...
    async init_listview() {
        ...
        if(listview){
            ...
            this.fixed_cols = []
            if (this.listview?.setup?.model == "Account"){
                this.fixed_cols.push({
                    column_title: `Account No`,
    ...
    #init_listview_filters() {
        $(".filters-wrapper").addClass("border border-default/70 border-dotted")
        ...
        if (!lite.utils.is_empty_array(this.listview?.filters) && !lite.utils.is_empty_array(this.$filters_wrapper)) {
            ...
            if (this.$filter_fields_wrapper) {
                ...
                $.each(this.listview?.filters, (_, f) => {
                    ...
                    this.$filter_fields_wrapper.append(this.html_builder.build_form_field(f, this.config.setup))
                })
    ...
    async populate_list(clear_listview=true) {
        if(clear_listview){
            this.#add_or_remove_list_loader()
        }
        this.#update_list_filters()

        return await lite.connect.get({
            model: this.listview.setup.model,
            ...
            if (resolve) {
                ...
                if (resolve.status === lite.status_codes.ok) {
                    ...
                    this.#generate_rows(data.rows, clear_listview)
                }
                else{
    ...
    #generate_rows(data,clear_listview=true) {
        let list_rows = ''
        if(clear_listview){
        ...
        if(this.current_page > 1){
            count_start = lite.utils.string_to_int((this.current_page-1) * this.page_size)
        }
        ...
        this.$list_body.append(list_rows)
    ...
    #update_list_filters() {
        ...
        if (!lite.utils.is_empty_array(filter_fields)) {
            ...
            $.each(filter_fields, (_, f) => {
                filters[lite.utils.get_attribute(f, 'fieldname')] = lite.utils.get_attribute(f, "value") || lite.utils.get_field_value(f)
            })

...
export default class Staff_HTML_Generator{
    ...
    emp_files (data) {
        ...
        $.each (data, (_, ele) => {
            card += `
                <div class="bg-white/40 rounded-md flex items-center justify-center flex-col">
                    <span class="material-symbols-outlined text-25 text-white">folder</span>
                    <small class="truncate overflow-ellipsis w-[90%] text-center">${ele.file_name}</small>
                </div>
            `
        })
    ...
    emp_recent_payslip (data) {
        ...
        $.each (data, (_, ele) => {
            ...
            console.log('====================================');
            list += `
                <div class="w-full rounded-sm h-full grid grid-cols-10 bg-pink-100 p-1">
                    <div class="w-full h-full col-span-4 flex flex-col items-start justify-center">
                        <span class="font-semibold">${ele?.name}</span>
                        <small class="font-semibold text-orange-800 text-10">${ele?.from_date} -> ${ele?.to_date}</small>
                    </div>
                    <div class="col-span-4 h-full w-full flex items-start justify-center flex-col">
                        <span class="net font-semibold text-13">${ele?.net}</span>
                        <small class="text-slate-700 text-10">Total Net</small>
                    </div>
                    <div class="col-span-2 h-full w-full flex items-center justify-end">
                        <button payslip-download-btn="${ele?.id}" class="bg-pink-600 w-[150px] h-[25px] flex items-center justify-center rounded-md text-12 text-white">
                            <span class="material-symbols-outlined mr-1 text-17">download</span>
                            Download
                        </button>
                    </div>
                </div>
    ...
    emp_todays_tasks (data) {
        let list = ''
        $.each (data, (_, ele) => {
            if (_ < 7) {
                list += `
                    <div class="w-full pending-tasks-today">
                        <div class="w-full grid grid-cols-13 gap-x-1 h-[50px] rounded-md overflow-hidden">
                            <div class="w-full h-full bg-default col-span-2 flex items-center justify-center text-white">
                                <span class="material-symbols-outlined text-24">timelapse</span>
                            </div>
                            <div class="w-full h-full bg-default/10 text-default px-2 items-start justify-center col-span-11 border border-default/50 border-dotted overflow-hidden rounded-r-md flex flex-col">
                                <span class="text-default overflow-ellipsis truncate text-11 font-semibold">${ele?.title}</span>
                                <div class="w-full flex items-center justify-between">
                                    <small class="text-orange-600 text-10">Due: ${ele?.expected_completion_time}</small>
                                    <small class="text-orange-600 text-10 bg-default text-white rounded-full px-3">${ele?.status}</small>
                                </div>
                            </div>
                        </div>
                    </div>
